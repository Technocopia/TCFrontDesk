...
class CalendarAgenda extends React.Component {
  render = () => {
    ...
    return <div className={`${preface}`}>
      ...
      <Table className={`${preface}-event-table`}><tbody>
        ...
        <tr key={index}>
          <td className={`${preface}-event-date`} align="right">
            ...
            <div className="time">{new Date(event.start).toLocaleTimeString(DATE_LOCALE, TIME_OPTIONS)} - {new Date(event.end).toLocaleTimeString(DATE_LOCALE, TIME_OPTIONS)}</div>
          </td>
